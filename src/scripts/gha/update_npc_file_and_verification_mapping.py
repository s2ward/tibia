...
def remove_status(mapping, file_path):
    ...
    if parts[0] == 'text':  # Adjust for the new directory structure
        parts = parts[1:]  
    cur = mapping
    for part in parts[:-1]:
        cur = cur[part]
...
def process_renamed_files(mapping, base_dir, repo_dir, renamed_files):
    ...
    for old_path, new_path in renamed_file_pairs:
        old_path = Path(old_path).relative_to('text')
        new_path = Path(new_path).relative_to('text')
        ...
        status = get_status(base_dir, repo_dir, repo_dir / 'text' / new_path)
        print(f"Status: {status}")
        if status != "UNCHANGED":
