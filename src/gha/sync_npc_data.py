...
if len(sys.argv) != 3:
    print("Usage: script.py <tibia/npc directory> <api/npc_data.json>")
    sys.exit(1)
...
def update_json_file(json_file_path, npc_structure):
    ...
    # Add any new NPCs found in the directory structure but not in the JSON
    # for name, details in npc_structure.items():
    #     data.append({
    #         "name": name,
    #         "location": details["area"],
    #         "subarea": details["subarea"] if details["subarea"] else '',
    #         # Fill in other default values as necessary
    #     })
    #     updated = True
